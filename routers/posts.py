...
from utils.faiss_store import add_embedding
...
async def create_new_post(
    ...
    # --- NEW: Add embedding for vector search ---
    try:
        response = requests.get(db_post.image_url)
        if response.status_code == 200:
            img_bytes = response.content
            embedding = get_image_embedding(img_bytes)
            add_embedding(db_post.id, embedding)
    except Exception as e:
        print(f"[WARNING] Failed to embed image for post {db_post.id}: {e}")
